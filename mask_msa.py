...
def remove_gaps(ref, msa_original):
        msa_edited = msa[:, 0:0] # Empty MSA (no sequences, only headers)
...
masked_msa = msa
for order in orders:
        reference = str(masked_msa[order].seq)
        masked_msa = remove_gaps(reference, masked_msa)

print('Writing to file...')

AlignIO.write(masked_msa, outfile, "fasta")
