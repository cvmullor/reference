...
do
	...
	# average coverage
	awk -v filename=$bamfile.nozero.temp '{ sum += $0; n++ } END { if (n > 0) print (sum / n) * 2" "filename; }' $bamfile.nozero.temp >> mcov_nozero.csv
	
	## All positions
	samtools depth -aa $bamfile.sorted.bam | awk '{ if ($3>=0) {print $3} }' > $bamfile.all.temp
	awk -v filename=$bamfile.all.temp '{ sum += $0; n++ } END { if (n > 0) print (sum / n) * 2" "filename; }' $bamfile.all.temp >> mcov_all.csv
done
